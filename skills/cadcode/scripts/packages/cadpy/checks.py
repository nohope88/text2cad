...
but the generator does not:
  ...
  * ``interference`` — advisory (``severity: "warning"``): two assembled parts
    overlap by more than mating fuzz but by too little of the smaller part's
    volume to be a gross ``collision``. This is the shape a 0.5mm mating error
    takes on a large part, which the fraction test alone lets through.
...
# interpenetration.
#
# The fraction is measured against the smaller part's *whole* volume, so it
# scales with part size while a mating error does not: a boss poking 0.5mm
# through a wall overlaps a few hundred mm^3, which is a fraction of a percent
# of a 40cm^3 part and clears the floor while never reaching 10%. Overlaps
# between the two thresholds are therefore reported as ``interference``
# (severity ``warning``) rather than dropped — visible to the review loop,
# non-blocking so an intended press fit does not fail a build.
...
def check_collisions(scene: object) -> list[Warning]:
    ...
    wall or two mispositioned parts. An overlap that clears the absolute floor
    but not the fraction is reported as ``interference`` (severity ``warning``):
    on a large part that band is where a sub-millimetre mating error lands, and
    dropping it silently is how one ships. Never raises.
    """
    import itertools
    ...
    for (name_a, shape_a, box_a, vol_a), (name_b, shape_b, box_b, vol_b) in (
        ...
        if smaller <= 0:
            continue
        if overlap < COLLISION_MIN_VOLUME_MM3:
            continue  # coincident mating faces and boolean fuzz
        if overlap >= COLLISION_MIN_FRACTION * smaller:
            warnings.append(
                _warning(
                    pair,
                    "collision",
            ...
            warnings.append(
                _warning(
                    pair,
                    "interference",
                    f"parts '{name_a}' and '{name_b}' overlap by {overlap:.1f} mm^3 "
                    f"({100.0 * overlap / smaller:.2f}% of the smaller part) in "
                    "assembled position — too much to be a shared mating face, too "
                    "little to be a gross collision. This is the size a "
                    "sub-millimetre mating error takes: a boss into a wall, a pin "
                    "into an undersized hole, a lid seated a fraction too deep. "
                    "Confirm it is an intended press fit; otherwise model the mate "
                    "at its stated clearance.",
                )
            )
    return warnings
